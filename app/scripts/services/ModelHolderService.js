...
function _qpop(key)
{
    ...
    var returnObj ={       
        set: function (key, value) {
            ...
            {
                ModelsService[key] = {__status:"success",__count:0,__requestedAt:new Date(), __receivedAt:new Date(),data:{}};
            }
        ...
        getByParams: function (key, reset, callback, params) {
            var self = this;
            if (reset || ModelsService[key] == undefined || JSON.stringify(ModelsService[key].requestParams) != JSON.stringify(ds[key].getRequestParams([params]))) { // it checks if request parameters are different than call again
